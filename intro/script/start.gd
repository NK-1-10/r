...
func _ready() -> void:
	...
	Signals.change_text.emit(intro[n])
...
func on_intro_finnished():
	...
	Global.Typewriter("Welcome!", $explain/Panel/header, false)
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton and waitingForClick:
		if click == 0 and Global.typing == false:
			Global.Typewriter("Great!", $explain/Panel/header, false)
			Global.Typewriter("Take your mouse and grab the package!", $explain/Panel/tut, false)
			click = click + 1
